...
- **Sources:** feeds on `*.substack.com` answer 403 to GitHub Actions; prefer a
  publication's own domain and test new feeds from Actions (`check_feed`). A
  source that can't be fetched is paused with `enabled: false` and a comment,
  never left failing every day. When a source's URL changes, check for repeats
  and migrate saved entries once (HOW_THIS_SITE_WORKS §11).
- **arXiv query and topic keywords:** measure any change on a real week of
  arXiv, reading the papers (relevant / doubtful / not), and never lose a
  clearly relevant paper. arXiv papers use stricter *alignment* keywords
  (`arxiv.topic_overrides`); blogs use the normal list (HOW_THIS_SITE_WORKS §5.2, §5.4).
...
- Pages must work **without JavaScript**; JS only adds extras (filters, tracker
  and My shelf, book dialog, theme toggle, Journal month selector, stale-data
  warning). Feed data is always autoescaped and external links go through
...
- ✅ **9: final documentation review.** All stages are done; ideas for later
  are in HOW_THIS_SITE_WORKS §12 ("Future ideas"). Don't start any of them
  without Marco asking.
- Before merging any experimental change, try it on a branch (see
  HOW_THIS_SITE_WORKS §10, "Trying a change on a branch").

## Folder structure
```
config/sources.yaml      all news sources (enabled: false = paused), arXiv query and
                         its topic_overrides, karma thresholds, topic keywords
...
scripts/fetch_news.py    fetch feeds + arXiv, dedupe, save JSON + candidates;
                         --dry-run reports only; --check-feed URL tests one feed
...
static/css, static/js    styles and small vanilla JS modules (filters, reading-store,
                         tracker, my-shelf, library, theme, journal, stale)
